...
def read_sweep_eval_results(results_dir):
    ...
    cols = ['graph_type', 'N', 'alpha', 'beta', 'seed']
    numeric_cols = ['N', 'alpha', 'beta', 'seed']
    ...
    for root, dirs, files in os.walk(results_dir):
        for file in files:
            if file == 'eval_results.txt':
                ...
                data['alpha'].append(parts[7].split('_')[-1])
                data['beta'].append(parts[8].split('_')[-1])
...
@hydra.main(version_base=None, config_path="config", config_name="config_best_sweep_mc_glsigrep")
