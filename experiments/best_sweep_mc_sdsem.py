...
def read_sweep_eval_results(results_dir):
    ...
    cols = ['graph_type', 'N', 'forget', 'lamb', 'seed']
    numeric_cols = ['N', 'forget', 'lamb', 'seed']
    ...
    for root, dirs, files in os.walk(results_dir):
        for file in files:
            if file == 'eval_results.txt':
                ...
                data['forget'].append(parts[7].split('_')[-1])
                data['lamb'].append(parts[8].split('_')[-1])
...
@hydra.main(version_base=None, config_path="config", config_name="config_best_sweep_mc_glasso")
