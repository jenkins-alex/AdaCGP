...
class AdaCGP:
    ...
    def run(self, X, y, weight_matrix=None, filter_coefficients=None, graph_filter_matrix=None, **kwargs):
        ...
        graph_filter_matrix = np.array(graph_filter_matrix) if graph_filter_matrix is not None else None
        filter_coefficients = np.array(filter_coefficients) if filter_coefficients is not None else None

        iter_range = range(process_length) if self._train_steps_list is None else self._train_steps_list
